...
export default function ManageUsersForAdmin({ users = [] }: ManageUsersProps) {
  ...
  const openActionDialog = (user: UserData, type: "delete" | "block" | "unblock" | "makeAdmin" | "makeUser") => {
      ...
      switch (dialogType) {
        case "delete":
          ...
          if (result.success !== false) {
            toast.success(`${selectedUser.name} has been deleted successfully.`, { id: toastId })
        ...
        case "block":
          ...
          if (result.success !== false) {
            toast.success(`${selectedUser.name} has been blocked.`, { id: toastId })
            // Refresh the page to get updated data
            router.refresh()
          } else {
        ...
        case "unblock":
          ...
          if (result.success !== false) {
            toast.success(`${selectedUser.name} has been activated.`, { id: toastId })
            // Refresh the page to get updated data
            router.refresh()
          } else {
        ...
        case "makeAdmin":
          ...
          if (result.success !== false) {
            toast.success(`${selectedUser.name} is now an admin.`, { id: toastId })
            // Refresh the page to get updated data
            router.refresh()
          } else {
        ...
        case "makeUser":
          ...
          if (result.success !== false) {
            toast.success(`${selectedUser.name} is now a regular user.`, { id: toastId })
            // Refresh the page to get updated data
            router.refresh()
          } else {
            throw new Error(result.error || "Failed to make admin a regular user")
          }
          break
      }
    } catch (error) {
                        ...
                        <TableCell className="text-right">
                          <div className="flex justify-end items-center gap-2">
                            ...
                            {user.role === "USER" ? (
                              ...
                              >
                                {actionLoading === user.id ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : null}
                                Make Admin
                              </Button>
                            ) : (
                              <Button
                                size="sm"
                                variant="outline"
                                onClick={() => openActionDialog(user, "makeUser")}
                                disabled={!!actionLoading}
                                className="whitespace-nowrap"
                              >
                                {actionLoading === user.id ? <Loader2 className="h-4 w-4 animate-spin mr-1" /> : null}
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button
              variant={dialogType === "delete" || dialogType === "block" ? "destructive" : "default"}
