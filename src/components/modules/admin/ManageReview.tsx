...
export default function ManageReviews({
  ...
  const filteredReviews = useMemo(() => {
    return initialData.filter((review) => {
      ...
      if (
        selectedPremium !== undefined &&
        review.isPremium !== selectedPremium
      ) {
        return false;
      }
      ...
      <div className="light:bg-white rounded-lg shadow p-4 mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant={
                    selectedPremium !== undefined ? 'default' : 'outline'
                  }
                  className={`cursor-pointer ${
                    selectedPremium !== undefined
                      ? 'light:bg-primary text-primary-foreground'
        ...
        >
          <TabsList className="mb-4 flex items-center gap-4">
            ...
            <TabsTrigger
              value="published"
              className="cursor-pointer light:light:bg-green-500 text-white"
            >
              Published
            </TabsTrigger>
            <TabsTrigger
