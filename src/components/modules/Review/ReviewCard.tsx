    ...
    >
      <CardHeader className="pb-2">
        <div className="flex justify-between items-start">
          <div>
            <h3 className={cn("font-semibold text-lg line-clamp-2 transition-colors", isHovered ? "text-primary" : "")}>
              {review.title}
            </h3>
      ...
      <CardContent className="py-2 flex-grow">
        ...
        <p className={cn("text-sm mt-3", review.isPremium ? "line-clamp-2 blur-[2px]" : "line-clamp-3")}>
          {review.excerp}
        </p>
        {review.isPremium && (
          <div className="mt-4" onClick={handleInnerClick}>
            <Button variant="outline" size="sm" className="w-full text-xs hover:bg-amber-50 transition-colors">
